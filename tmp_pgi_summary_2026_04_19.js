...
(async()=>{
 ...
 const pairAgg={};
 for(const p of pairs){ const k=`${p.region_a}__${p.region_b}`; (pairAgg[k] ||= {a:p.region_a,b:p.region_b,vals:[]}).vals.push(Number(p.pair_pgi)); }
 console.log(JSON.stringify({
  ...
  topPairs:Object.values(pairAgg).map(x=>({a:x.a,b:x.b,avg:round(x.vals.reduce((m,n)=>m+n,0)/x.vals.length),count:x.vals.length})).sort((a,b)=>b.avg-a.avg).slice(0,10)
 },null,2));
})();