...
const SCAN_PATH = '/Users/treelight/.openclaw/workspace/memory/scans/2026-04-27-am.md';
const OUTPUT_PATH = '/Users/treelight/.openclaw/workspace/memory/scans/2026-04-27-am-scores.json';
const ENV_PATH = path.join(__dirname, '.env.local');
const SCAN_DATE = '2026-04-27';
...
function scorePGI(story) {
  ...
  let d5 = base + ((cat === 'governance' || cat === 'legal' || cat === 'tech-ai' || cat === 'sanctions') ? 0.8 : 0) + (patterns.has('divergence') ? 0.6 : 0) + (patterns.has('consensus') ? -0.7 : 0);
  let d6 = base + ((cat === 'sanctions' || cat === 'security' || cat === 'economic-flows' || cat === 'tech-ai' || cat === 'governance') ? 1 : 0) + (sig >= 4 ? 0.6 : 0) + (patterns.has('consensus') ? -0.8 : 0);
...
(async()=>{
  ...
  for (const story of rawStories) {
    ...
    const regions_covered = [...new Set(regions_found.filter(r => r !== 'global'))];
    ...
    for (let i = 0; i < pairRegions.length; i++) {
      for (let j = i + 1; j < pairRegions.length; j++) {
        ...
        storyPairs[`${region_a}_${region_b}`] = pair_pgi;
      }
    }
    ...
    scored.push({
      ...
      category: story.category,
      significance,
      significance_label: story.significance,
      regions_found,
      regions_absent,
      regions_covered,
      base: story.perception_gap,
      story_pgi: pgi.story_pgi,
      d1_factual: pgi.d1_factual,
      d2_causal: pgi.d2_causal,
      d3_framing: pgi.d3_framing,
      d4_emotional: pgi.d4_emotional,
      d5_actor_context: pgi.d5_actor_context,
      d6_cui_bono: pgi.d6_cui_bono,
      coverage_breadth: regions_covered.length,
      d1_coverage_breadth: gai.d1_coverage_breadth,
      d2_prominence_disparity: gai.d2_prominence_disparity,
      d3_population_exposure: gai.d3_population_exposure,
      d4_significance_severity: gai.d4_significance_severity,
      story_gai: gai.story_gai,
      region_pairs: storyPairs,
      scoring_rationale,
    });
  }
  ...
  const out = {
    ...
    pgiCount: pgiStories.length,
    pairCount: pairRows.length,
    gaiCount: gaiStories.length,
    scored,
  };
