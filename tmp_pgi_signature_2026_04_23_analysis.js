...
(async()=>{
 ...
 const pairAgg={};
 for(const p of pairs){ const k=[[p.region_a,p.region_b].sort().join('__'), p.story_headline].join('||'); (pairAgg[k] ||= {a:p.region_a,b:p.region_b,vals:[],story:p.story_headline}).vals.push(Number(p.pair_pgi)); }
 ...
 console.log(JSON.stringify({
  ...
  categories:Object.entries(cats).map(([category,items])=>({category,pgi:round(weightedAvg(items,'story_pgi')),count:items.length,top:[...items].sort((a,b)=>b.story_pgi-a.story_pgi).slice(0,3).map(x=>x.story_headline)})).sort((a,b)=>b.pgi-a.pgi),
  ...
  topStories:stories.slice(0,10).map(s=>({headline:s.story_headline,pgi:round(s.story_pgi),slug:s.story_slug,category:s.category,regions:s.regions_covered,scan_period:s.scan_period,significance:s.significance,rationale:s.scoring_rationale,d1:round(s.d1_factual),d2:round(s.d2_causal),d3:round(s.d3_framing),d4:round(s.d4_emotional),d5:round(s.d5_actor_context ?? s.d5_actor),d6:round(s.d6_cui_bono)})),
  topPairs:Object.values(pairAgg).map(x=>({a:x.a,b:x.b,avg:round(mean(x.vals)),story:x.story})).sort((a,b)=>b.avg-a.avg).slice(0,15)
 },null,2));
})();
