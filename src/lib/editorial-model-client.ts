...
function messagesToOpenClawPrompt(messages: EditorialMessage[], responseSchema?: unknown): string {
  ...
  const schemaHint = responseSchema
    ? `\n\nReturn valid JSON only. Required shape: {overview:string, source_note:string, observations:string[2-4], topics: exactly 10 objects with cluster_id, topic_label, headline, paragraphs:string[1-2], source_ids:string[2-5], perception_gap:{view, where_the_split_appears, why_it_matters, score}}. No markdown.`
