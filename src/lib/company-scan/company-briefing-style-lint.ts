...
const BANNED_PHRASES_BLOCKING: Array<{ pattern: RegExp; label: string }> = [
  ...
  { pattern: /\bstakeholders\s+should\s+(monitor|watch|observe|keep\s+an\s+eye)\b/i, label: "stakeholders should monitor" },
  ...
  { pattern: /\bin\s+today'?s\s+(rapidly\s+)?(evolving|changing|fast[- ]paced)\b/i, label: "in today's rapidly evolving" },
  { pattern: /\bit\s+is\s+important\s+to\s+note\b/i, label: "it is important to note" },
  { pattern: /\bmarks?\s+a\s+pivotal\s+(moment|shift|turning\s+point)\b/i, label: "marks a pivotal moment" },
  { pattern: /\bmust\s+act\s+now\b/i, label: "must act now" },
  { pattern: /\burgent\s+action\s+required\b/i, label: "urgent action required" },
  { pattern: /\bcan'?t\s+afford\s+to\s+ignore\b/i, label: "can't afford to ignore" },
...
const RESTRICTED_PHRASES_WARNING: Array<{ pattern: RegExp; label: string }> = [
  { pattern: /\bunderscores?\s+(the\s+)?(importance|need|urgency|significance)\b/i, label: "underscores the importance" },
  { pattern: /\bhighlights?\s+(the\s+)?(importance|need|urgency|growing)\b/i, label: "highlights the importance" },
  { pattern: /\bshowcases?\b/i, label: "showcases" },
  { pattern: /\bsignals?\s+(a\s+)?(broader|wider|growing|important)\b/i, label: "signals a broader" },
  { pattern: /\bamid\s+(growing|increasing|rising|heightened|ongoing)\b/i, label: "amid growing" },
 ...
 */
export function lintBriefingStyle(output: CompanyBriefingGenerationOutput): StyleLintResult {
    ...
    for (const pattern of PROMPT_LEAKAGE_PATTERNS) {
      if (pattern.test(seg.text)) {
        ...
        issues.push({
          ...
          location: seg.path,
          text: match?.[0] ?? seg.text.slice(0, 80),
          message: "Raw prompt or source instruction detected in output. Must be removed before send.",
        });
      }
    }
  }
  ...
  for (const seg of segments) {
    ...
    if (matches && matches.length > 0) {
      issues.push({
        ...
        location: seg.path,
        text: matches.join(", "),
        message: "Source-name clutter in body text. Prefer clean prose; attribute in source notes.",
      });
    }
  }
  ...
  const prohibitedFound = [
    ...new Set(
      issues
        .filter((i) => i.code === "BANNED_PHRASE" || i.code === "RESTRICTED_PHRASE")
        .map((i) => i.text),
    ),
  ];
  ...
  return {
    ...
    no_hype: issues.filter((i) =>
      ["BANNED_PHRASE", "RESTRICTED_PHRASE"].includes(i.code) &&
      ["shocking", "explosive", "bombshell", "panic", "chaos", "game-changing", "crisis"].some(
        (w) => i.text.toLowerCase().includes(w),
      ),
    ).length === 0,
    ...
    result: blockingIssues.length > 0 ? "block" : warningIssues.length > 0 ? "warn" : "pass",
    issues,
  };
}
...
function extractTextSegments(output: CompanyBriefingGenerationOutput): TextSegment[] {
  const segments: TextSegment[] = [];

  // Today's Brief
  if (output.today_brief.top_line.text) {
    segments.push({ path: "today_brief.top_line", text: output.today_brief.top_line.text });
  }
  ...
  for (let si = 0; si < output.main_briefing.sections.length; si++) {
    ...
    for (let ii = 0; ii < section.items.length; ii++) {
      ...
      segments.push({ path: `sections[${si}].items[${ii}].title`, text: item.title.text });
      segments.push({ path: `sections[${si}].items[${ii}].body`, text: item.body.text });
      if (item.why_it_matters?.text) {
        segments.push({ path: `sections[${si}].items[${ii}].why_it_matters`, text: item.why_it_matters.text });
      }
      if (item.uncertainty_line?.text) {
        segments.push({ path: `sections[${si}].items[${ii}].uncertainty_line`, text: item.uncertainty_line.text });
      }
      if (item.perception_gap_note?.text) {
        segments.push({ path: `sections[${si}].items[${ii}].perception_gap_note`, text: item.perception_gap_note.text });
      }
      if (item.source_attribution?.text) {
        segments.push({ path: `sections[${si}].items[${ii}].source_attribution`, text: item.source_attribution.text });
      }
    }
  }
  ...
  for (let i = 0; i < output.perception_gap.notes.length; i++) {
    segments.push({ path: `perception_gap.notes[${i}]`, text: output.perception_gap.notes[i].note.text });
  }

  // Useful Observations
  for (let i = 0; i < output.useful_observations.observations.length; i++) {
    segments.push({ path: `useful_observations[${i}]`, text: output.useful_observations.observations[i].text });
  }

  // Source Notes
  if (output.source_notes.text.text) {
    segments.push({ path: "source_notes", text: output.source_notes.text.text });
  }
...
function detectRepeatedPhrasing(segments: TextSegment[]): RepeatedPhrase[] {
  ...
  const phraseCounts = new Map<string, { count: number; locations: string[] }>();

  for (const seg of segments) {
    const words = seg.text.toLowerCase().split(/\s+/).filter((w) => w.length > 0);
    for (let n = 3; n <= 4; n++) {
...
function isCommonPhrase(phrase: string): boolean {
  const common = new Set([
    "in the", "of the", "to the", "for the", "on the", "at the",
    "is a", "is the", "it is", "it was", "has been", "have been",
    "this is", "that is", "which is", "there is", "there are",
    "will be", "would be", "could be", "may be", "might be",
    "as a", "as the", "by the", "with the", "from the",
    "and the", "but the", "or the", "not the",
    "in a", "of a", "to a", "for a",
  ]);
