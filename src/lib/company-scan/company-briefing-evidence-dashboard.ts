...
function section(title: string, body: string): string {
  return `<section id="${esc(
    title
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-+|-+$/g, ""),
  )}" style="margin-top:28px;border:1px solid ${BORDER};border-radius:22px;background:white;padding:24px;">
...
export function generateCompanyEvidenceDashboardHtml(
  doc: CompanyBriefingEvidenceDocument,
): string {
  const scan = doc.scan_summary;
  const sourceMix = doc.source_quality_summary.source_mix;
  const scanWindow = scan.scan_window
    ? `${scan.scan_window.from} → ${scan.scan_window.to}`
    : "Not recorded";
  ...
  const summaryCards = [
    ...
    statCard(
      "Selected",
      scan.selected_for_email,
      "Included in the email briefing",
    ),
    statCard(
      "Dashboard coverage",
      scan.dashboard_only_count,
      "Relevant context kept out of the main email",
    ),
    statCard(
      "Excluded/noise",
      scan.excluded_count,
      "Duplicates, sludge, stale items, or poor matches",
    ),
    statCard("Source domains", scan.all_source_domains_count),
    statCard("Key domains", scan.key_source_domains_count),
  ].join("");

  const briefingEvidence = doc.briefing_sections
    .map(
      (item) => `
      ...
      <details style="margin-top:10px;">
        ...
        <ul style="margin:8px 0 0 18px;padding:0;font:400 13px/1.55 -apple-system,BlinkMacSystemFont,sans-serif;color:${BODY};">
          ${item.claims
            ...
            .map(
              (claim) =>
                `<li>${esc(claim.text)} <span style="color:${MUTED};">(${esc(claim.claim_type)}, confidence ${pct(claim.confidence)})</span></li>`,
            )
            .join("")}
        </ul>
  ...
  const excluded = Object.entries(doc.excluded_summary.counts_by_reason || {})
    .map(
      ([reason, count]) =>
        `<li><strong>${esc(reason.replace(/_/g, " "))}:</strong> ${esc(count)}</li>`,
    )
    .join("");
  ...
  const tabs = [
    ...
    tab("Daily scan evidence", "#briefing-evidence"),
...
<head>
  ...
  <title>${esc(doc.company_name)} Source Trail — ${esc(doc.scan_date)}</title>
</head>
<body style="margin:0;background:${BG};color:${BODY};">
  <main style="max-width:1040px;margin:0 auto;padding:36px 20px 56px;">
    <p style="margin:0 0 8px;font:700 11px/1.4 -apple-system,BlinkMacSystemFont,sans-serif;color:${AMBER};letter-spacing:2px;text-transform:uppercase;">Albis source trail</p>
    ...
    <p style="margin:12px 0 0;max-width:760px;font:400 15px/1.65 -apple-system,BlinkMacSystemFont,sans-serif;color:${MUTED};">This page shows the wider source trail behind the daily scan: what was found, what was held back, and how the evidence was classified. It is designed for verification, not for quick reading.</p>
    ...
    ${section("Daily scan evidence", briefingEvidence)}
