      ...
      <div className="mt-8 grid gap-5">
        {filtered.length ? filtered.map((story, index) => (
          <article key={`${story.headline}-${index}`} className="rounded-3xl border border-black/[0.07] bg-white p-6 dark:border-white/[0.07] dark:bg-white/[0.03]">
            ...
            <Link href="/what-am-i-missing" className="mt-5 inline-flex h-10 items-center justify-center rounded-full bg-[#c8922a] px-5 text-sm font-semibold text-white hover:bg-[#b17f24]">
              Explore this gap
            </Link>
          </article>
        )) : (
