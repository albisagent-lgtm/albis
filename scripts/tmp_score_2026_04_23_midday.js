...
const SCAN_DATE = '2026-04-23';
const SCAN_PERIOD = 'midday';
const scanPath = path.join(__dirname, '..', '..', 'memory', 'scans', '2026-04-23-midday.md');
const outPath = path.join(__dirname, '..', '..', 'memory', 'scans', '2026-04-23-midday-scores.json');
...
const scored = stories.map((story) => {
  ...
  const text = `${headline} ${story.connection || ''}`.toLowerCase();
  ...
  if (/ceasefire|mediat|extension|talks/.test(text)) {
    d2 += 0.3; d3 += 0.5; d5 += 0.4; d6 += 0.4;
  }
  if (/hormuz|shipping|oil|sanctions|pipeline|transit/.test(text)) {
    d2 += 0.2; d3 += 0.2; d6 += 0.4;
  }
  if (/vaccine|outbreak|health|cdc|covid/.test(text)) {
    d4 += 0.3; d5 += 0.2;
  }
  if (/okinawa|naval|japan|china/.test(text)) {
    d3 += 0.4; d5 += 0.3; d6 += 0.2;
  }
  if (/foot-and-mouth|outbreak/.test(text)) {
    d1 -= 0.3; d2 -= 0.2; d3 -= 0.4; d5 -= 0.2;
  }
  if (/cdc blocks publication|health governance|scientific report/.test(text)) {
    d3 += 0.5; d4 += 0.4; d5 += 0.4; d6 += 0.3;
  }
  ...
  let d4g = sig === 'critical' ? 9.2 : sig === 'high' ? 8.3 : sig === 'medium' ? 7.0 : 5.8;

  if (/hormuz|oil|sanctions|pipeline/.test(text)) d4g += 0.4;
  if (/vaccine|health|cdc|covid/.test(text)) d4g += 0.3;
  if (/okinawa|naval|japan|china/.test(text)) d2g += 0.3;
  if (/foot-and-mouth/.test(text)) d2g -= 0.2;
...
async function main() {
  const { data: badPgiRows, error: fetchPgiError } = await supabase
