...
const scanPath = path.join(__dirname, '..', '..', 'memory', 'scans', '2026-04-23-pm.md');
const outPath = path.join(__dirname, '..', '..', 'memory', 'scans', '2026-04-23-pm-scores.json');
...
const scored = stories.map((story) => {
  ...
  const text = `${headline} ${story.connection || ''} ${(story.tags || []).join(' ')}`.toLowerCase();
  ...
  if (/ceasefire|mediat|extension|talks|unclear/.test(text)) {
    d2 += 0.4; d3 += 0.6; d5 += 0.4; d6 += 0.4;
  }
  if (/hormuz|shipping|ship|oil|sanctions|loan|veto|tariff|trade/.test(text)) {
    d2 += 0.2; d3 += 0.2; d6 += 0.4;
  }
  if (/iran|israel|ukraine|russia/.test(text)) {
    d3 += 0.3; d5 += 0.2;
  }
  if (/markets|shares|earnings/.test(text)) {
    d1 -= 0.2; d4 -= 0.2;
  }
  if (/ai|openai|oracle|cerebras|chips|cloud|compute/.test(text)) {
    d1 -= 0.4; d4 -= 0.2; d5 += 0.1;
  }
  if (/migration|border|channel crossings/.test(text)) {
    d3 += 0.1; d5 += 0.1;
  }
  ...
  if (/hormuz|oil|sanctions|shipping|ukraine|russia/.test(text)) d4g += 0.4;
  if (/ai|chips|compute/.test(text)) d4g -= 0.2;
  if (/migration|border/.test(text)) d4g -= 0.1;
