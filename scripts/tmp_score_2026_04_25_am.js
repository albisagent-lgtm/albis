...
const scanPath = '/Users/treelight/.openclaw/workspace/memory/scans/2026-04-25-am.md';
const outPath = '/Users/treelight/.openclaw/workspace/memory/scans/2026-04-25-am-scores.json';
const scanDate = '2026-04-25';
const scanPeriod = 'am';

const REGION_MAP = {
  'us': { full: 'us', short: 'us', pop: 380 },
  'europe': { full: 'europe', short: 'eu', pop: 750 },
  'middle-east': { full: 'middle-east', short: 'me', pop: 500 },
  ...
  'pacific': { full: 'pacific', short: 'pc', pop: 45 },
  'caribbean': { full: 'caribbean', short: 'cb', pop: 44 },
...
function pairBias(a, b, category, headline) {
  const pair = [a,b].sort().join('|');
  let bias = 0;
  if (pair === 'eu|us') bias += 0.2;
  if (pair === 'me|us') bias += 1.0;
  if (pair === 'eu|me') bias += 0.8;
  if (pair === 'sa|us') bias += 0.9;
  if (pair === 'af|eu') bias += 0.6;
  if (pair === 'la|us') bias += 0.7;
  if (pair === 'ap|us') bias += 0.8;
  if (pair === 'ap|eu') bias += 0.4;
  if (/migration|social|legal/.test(category)) bias += 0.3;
  if (/tech-ai|trade/.test(category)) bias += 0.5;
  if (/ceasefire|talks|iran|detention|deepseek|huawei|border|sanctions/i.test(headline)) bias += 0.3;
  return bias;
}

function makeDimensions(base, category, headline) {
  let d1 = base - 0.9;
  let d2 = base + (/conflict|sanctions|economic-flows|trade|governance/.test(category) ? 0.4 : 0.2);
  let d3 = base + (/diplomacy|security|migration|social|legal/.test(category) ? 0.7 : 0.5);
  let d4 = base + (/conflict|social|migration|food-agriculture/.test(category) ? 0.6 : 0.2);
  let d5 = base + (/conflict|tech-ai|trade|migration/.test(category) ? 0.5 : 0.3);
  let d6 = base + (/economic|economic-flows|sanctions|trade|governance/.test(category) ? 0.6 : 0.3);
  if (/ceasefire|talks|loan|arrested|jails|approves|advances/i.test(headline)) d1 += 0.1;
...
function gaiDimensions(item) {
  const breadthRaw = Number(item.coverage_breadth || (item.regions_found || []).length || 1);
  const found = (item.regions_found || []).map(fullRegion);
  const absent = (item.regions_absent || []).map(fullRegion);
...
function rationale(item, dims, gai) {
  return `${item.connection} PGI is driven mainly by differences in causal attribution, narrative framing, and actor portrayal across ${item.regions_found.join(', ')} coverage. GAI reflects coverage across ${gai.found.length} regions, absence in ${gai.absent.length}, and the size of populations likely to miss the story.`;
}
...
async function main() {
  ...
  for (const item of items) {
    ...
    const dims = makeDimensions(base, item.category, item.headline);
    const gai = gaiDimensions(item);
    const regionsCovered = (item.regions_found || item.regions || []).map(shortRegion).filter(Boolean);
    const pair_pgi = {};
    for (let i = 0; i < regionsCovered.length; i++) {
      for (let j = i + 1; j < regionsCovered.length; j++) {
        ...
        pair_pgi[[a,b].sort().join('|')] = r1(clamp(base + pairBias(a,b,item.category,item.headline)));
      }
    }
    ...
    const story = {
      ...
      coverage_breadth: item.coverage_breadth,
      dimensions: {
    ...
    for (const [pairKey, pairScore] of Object.entries(story.pair_pgi)) {
      ...
      const { error: pairError } = await supabase
        ...
        .upsert({ story_score_id: pgiInserted.id, region_a, region_b, pair_pgi: pairScore, scan_date: scanDate }, { onConflict: 'story_score_id,region_a,region_b', ignoreDuplicates: false });
    ...
    const { error: gaiError } = await supabase
      ...
      .upsert(gaiRow, { onConflict: 'story_slug,scan_date,scan_period', ignoreDuplicates: false });
